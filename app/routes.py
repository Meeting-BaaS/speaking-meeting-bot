...
from app.services.mcp_presets import resolve_mcp_config
...
async def join_meeting(request: BotRequest, client_request: Request):
    ...
    if prompt_context.block:
        resolved_persona_data["additional_content"] = merge_context_blocks(
            [
                resolved_persona_data.get("additional_content") or "",
    ...
    try:
        resolved_mcp = resolve_mcp_config(
            request.mcp_profile,
            request.mcp_profile_tool_access,
            request.mcp,
        )
    ...
    mcp_runtime_headers = []
    if resolved_mcp:
        mcp_payload, mcp_runtime_headers = split_mcp_runtime_headers(resolved_mcp)
        ...
        # Only attempt to generate image if custom prompt was originally used AND details were derived
            ...
            try:
                ...
                if generated_image_url:
                    ...
                    logger.error(
                        "Failed to generate image from prompt derived details: No URL returned."
                    )
                    bot_image = None
            except Exception as e:
                logger.error(
...
async def meetingbaas_webhook(request: Request):
    ...
    try:
        ...
        is_ready_event = any(
            ready in event_type_lower for ready in ready_events
        ) or any(ready in status_code_lower for ready in ready_events)
        ...
        is_end_event = any(end in event_type_lower for end in end_events) or any(
            end in status_code_lower for end in end_events
        )
    ...
    try:
        ...
        if bot_id:
            ...
            if os.path.exists(existing_summary):
                logger.info(
                    f"[WEBHOOK] Summary already exists for bot {bot_id}, skipping generation"
                )
                return
...
Format your response as JSON with these keys: prospect_name, company_name, summary, qualified, next_steps"""
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a helpful assistant that analyzes sales call transcripts. Always respond with valid JSON.",
                },
