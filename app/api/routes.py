...
from app.services.meetingbaas_api import create_meeting_bot, leave_meeting_bot
...
async def join_meeting(request: BotRequest, client_request: Request):
    ...
    prompt_derived_details = None  # Guard: always defined before elif reference
    ...
    if request.prompt: # Case 1: Custom prompt provided (dynamic persona)
        ...
        if prompt_derived_details and isinstance(prompt_derived_details, dict): # Ensure it's a dict
            ...
            resolved_persona_data = {
                "name": request.bot_name if request.bot_name else prompt_derived_details.get("name", "Bot"),
    ...
    if meetingbaas_bot_id:
        ...
        session_manager.store_session(
            ...
            meeting_url=request.meeting_url,
            marketing_person_email="frontend_user@example.com",
...
@router.get(
    ...
    responses={
        200: {"description": "Live session summary for the bot"},
        404: {"description": "No active session found for this bot ID"},
    },
)
async def get_bot_summary(bot_id: str):
    ...
    Pulls data from the in-memory SessionManager (god node) including
    transcriptions, notes, extracted needs, and current mode.
    Falls back to MEETING_DETAILS for bots that bypassed the sales flow.
    """
    from app.services.session_manager import session_manager

    # Try SessionManager first (sales-flow bots)
    session = session_manager.get_session_by_bot_id(bot_id)
    if session:
        # Get persona_name from MEETING_DETAILS if available
        persona_name = "Meeting Bot"
        for cid, details in MEETING_DETAILS.items():
    ...
    # Fallback: look in MEETING_DETAILS for non-sales bots
    ...
    raise HTTPException(
        ...
        detail=f"No active session found for bot_id={bot_id}",
    )
...
async def meetingbaas_webhook(request: Request):
    ...
    try:
        ...
        if event == "complete":
            ...
            if session and isinstance(transcript, list):
